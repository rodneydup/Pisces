...
// MAT 240B
//
// Copyright (C) 2017  Rodney DuPlessis <duplessis@umail.ucsb.edu>
...
struct App : AudioVisual, MIDI {
  ...
  // 	Timer timer;
  // 	SamplePlayer player = SamplePlayer("./01 DtSang start.wav");
  // 	float timey = 0.0f;
  ...
  void visual() {
    {
      ...
      // 			if ((int)message[i] == 128) off
      {
        ...
        if (compressionLFO1on) {
          ...
          ImGui::SliderFloat("Compression Ratio", &compressionLFO1, 0.0f, 2.0f);
        ...
        ImGui::VSliderFloat("##attack", ImVec2(40, 160), &attack, 1, 100,
                            "%.0f");
        ImGui::SameLine(60);
        ImGui::VSliderFloat("##decay", ImVec2(40, 160), &decay, 1, 1000,
                            "%.0f");
        ImGui::SameLine(120);
        ImGui::VSliderFloat("##sustain", ImVec2(40, 160), &sustain, 0, 1,
                            "%.3f");
        ImGui::SameLine(180);
        ImGui::VSliderFloat("##release", ImVec2(40, 160), &release, 1, 2000,
                            "%.0f");
        if (attack != wasAttack || decay != wasDecay || sustain != wasSustain ||
            release != wasRelease) {
        ...
        // * 0.75f);
        //       ImGui::SliderFloat("ADSR Timer", &timerPeriod, 0.01f, 2.0f);
        ...
        //       ImGui::SliderFloat("Sampler Player rate", &rate, -2.0f, 2.0f);
        //       rateLine.set(rate);
...

        {
          ...
          drawList->AddRectFilled(ImVec2(0, 308), ImVec2(windowWidth, 650),
                                  687855585, 0.0f, 0x0F);
          for (unsigned i = 0; i < 599;
          ...
          for (int i = 0; i < 10; i++) {
            ...
            char text[4];
            sprintf(text, "%ds", x);
            drawList->AddText(0, 10.0f,
                              ImVec2(i * (windowWidth / 10.0f) + 5.0f, 654.0f),
                              4294967295, text, NULL, 50.f, NULL);
          }
          drawList->AddText(0, 10.0f, ImVec2(2.0f, 30.0f), 3983212369, "LFO1",
                            NULL, 50.f, NULL);
          drawList->AddText(0, 10.0f, ImVec2(25.0f, 30.0f), 3983212369, "LFO2",
                            NULL, 50.f, NULL);
        }
        ...
        ImGui::PlotLines("Scope", &b[0], blockSize, 0, nullptr, FLT_MAX,
                         FLT_MAX, ImVec2(0, 70), 2 * sizeof(float));
        ImGui::PopItemWidth();
        ...
        ImGui::SameLine(1000);
        ImGui::Text("Copyright 2017 Rodney DuPlessis");
